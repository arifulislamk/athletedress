          ...
          {jerseys?.slice(0, 6).map((jersey) => { 
            ...
            >
              ...
              <div className="card-body space-y-3">
                <h2 className="font-poppins font-semibold md:font-bold md:card-title">
                  {jersey?.jerseyName}
                </h2>

                <div className="card-actions md:text-xl mt-14 justify-between items-center ">
                  <p>নিয়েছেন: : {jerseyQuantity}</p>
                  <p>
                    দাম: ৳<span className=" font-bold">{jersey?.price}</span>
                  </p>
                  <Link to={`/jerseyDetails/${jersey?._id}`}>
                      <button className=" bg-green-500 text-cyan-50 md:text-xl md:font-bold p-1 md:p-2 rounded-lg ">অর্ডার করুন</button>
                  </Link>
                </div>
              </div>
            </div>
